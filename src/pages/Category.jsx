...
export default function Category() {
  ...
  useEffect(()=>{
   async function fetchListings(){
     try {
      ...
      const q= query(listingRef, where("type", "==", params.categoryName),orderBy("timestamp","desc"),limit(6))
  ...
  async function onFetchMoreListing(){
    try {
      const listingRef = collection(db,"listings")
      const q= query(listingRef, where("type", "==", params.categoryName),startAfter(lastFetchListing),orderBy("timestamp","desc"),limit(6))
      const querySnap = await getDocs(q)
      const lastVisible= querySnap.docs[querySnap.docs.length - 1]
      setLastFetchListing(lastVisible)
      const listings = []
  ...
  return <div className="max-w-6xl mx-auto px-3">
     <h1 className="text-3xl text text-center mt-6 font-bold mb-6">{params.categoryName === "rent" ? "places for rent": "Place for sale"}</h1>
      {loading ? (
        <Spinner/>
